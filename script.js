...
// invoke methods on their buttons
    editButton.onclick = () => editNote(noteDiv);
    deleteButton.onclick = () => deleteNote(noteDiv);
    return noteDiv;
}

// returns note to previous value before "edit note" was clicked and edits made
...
// function to initiate ability to edit notes title and body via button
// editSave = false, prevents edit onclick event triggering for both edit and save buttons (infinitely editing scenario)
...
// function to trigger Save Note button
    if (editSave) {
        const note = notes.find(note => note.id == noteDiv.id);
...
// function to save a note to the top of the list (first in array)
const saveNote = () => {
